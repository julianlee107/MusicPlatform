# Generated by Django 2.0.3 on 2018-03-30 19:39

import datetime
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Music_sheet',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=100, verbose_name='歌单名称')),
                ('desc', models.TextField(verbose_name='歌曲描述')),
                ('create_time', models.DateField(default=datetime.datetime.now, verbose_name='创建时间')),
                ('fav_nums', models.IntegerField(default=0, verbose_name='收藏人数')),
                ('listen_nums', models.IntegerField(default=0, verbose_name='播放次数')),
                ('songs_nums', models.IntegerField(default=0, verbose_name='歌曲数')),
                ('sheet_tags', models.CharField(max_length=50, verbose_name='歌单标签')),
                ('sheet_image', models.ImageField(upload_to='music_sheets/%Y%m', verbose_name='歌单缩略图')),
            ],
            options={
                'verbose_name': '歌单',
                'verbose_name_plural': '歌单',
            },
        ),
        migrations.CreateModel(
            name='Singer',
            fields=[
                ('id', models.IntegerField(default=0, primary_key=True, serialize=False, unique=True, verbose_name='歌手编号')),
                ('name', models.CharField(default='匿名歌手', max_length=50, verbose_name='歌手')),
                ('singer_brief', models.TextField(verbose_name='歌手简介')),
                ('click_nums', models.IntegerField(default=0, verbose_name='点击数')),
                ('fav_nums', models.IntegerField(default=0, verbose_name='收藏人数')),
                ('add_time', models.DateTimeField(default=datetime.datetime.now, verbose_name='添加时间')),
            ],
            options={
                'verbose_name': '歌手',
                'verbose_name_plural': '歌手',
            },
        ),
        migrations.CreateModel(
            name='Songs',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=100, verbose_name='歌曲名')),
                ('song_detail', models.TextField(verbose_name='歌曲详情')),
                ('song_resourse', models.URLField(verbose_name='播放链接')),
                ('singer_name', models.ForeignKey(null=True, on_delete=django.db.models.deletion.CASCADE, to='music_sheet.Singer', verbose_name='歌手')),
            ],
            options={
                'verbose_name': '音乐',
                'verbose_name_plural': '音乐',
            },
        ),
        migrations.CreateModel(
            name='TypeDict',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('type', models.CharField(max_length=20, verbose_name='风格')),
                ('desc', models.CharField(max_length=200, verbose_name='描述')),
                ('add_time', models.DateField(default=datetime.datetime.now)),
            ],
            options={
                'verbose_name': '类别',
                'verbose_name_plural': '类别',
            },
        ),
        migrations.AddField(
            model_name='songs',
            name='song_type',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='music_sheet.TypeDict', verbose_name='音乐类型'),
        ),
    ]
